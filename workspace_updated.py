...
# ---------------------------------------------------------------------------
# Custom Ridgeback Franka modules (from warehouse_play.py)
# These must be importable from the same directory as this script.
# ---------------------------------------------------------------------------
from ridgeback_prim import RidgebackFranka
from ridgeback_pick_place import RidgeBackFrankaPickPlace


class WarehousePlaybook(object):
    """Ridgeback Franka pick-and-place playbook (ported from warehouse_play.py).

    Uses the real RidgebackFranka articulation robot with RidgeBackFrankaPickPlace
    controller instead of a visual-only simulated mobile base.

    State machine (same 3-event logic as warehouse_play.py):
      Event 0 – Drive Ridgeback toward the cube (moveinxdirection +1)
      Event 1 – Execute pick-and-place (arm forward until is_done)
      Event 2 – Drive Ridgeback back to start  (moveinxdirection -1)
    """

    def __init__(self, ridgebackcontroller: "RidgeBackFrankaPickPlace"):
        self.ridgebackcontroller = ridgebackcontroller
        self.event = 0
        self.ridge_targetlocation = [2.3, -1, 0]

    # ------------------------------------------------------------------
    # Core playbook logic
    # ------------------------------------------------------------------

    def play(self, testrun: bool = False):
        # ── Event 0: Move Ridgeback toward the object ───────────────────
        if self.event == 0:
            self.ridgebackcontroller.moveinxdirection(1)
            if self.ridgebackcontroller.reachedlocation(self.ridge_targetlocation):
                self.event += 1

        # ── Event 1: Pick up the object ─────────────────────────────────
        elif self.event == 1:
            self.ridgebackcontroller.forward()
            if self.ridgebackcontroller.is_done():
                self.event += 1

        # ── Event 2: Move Ridgeback back to starting position ────────────
        elif self.event == 2:
            self.ridgebackcontroller.moveinxdirection(-1)
            if self.ridgebackcontroller.reachedlocation([0.1, -1, 0]):
                self.event += 1

    def reset(self):
        self.event = 0
        self.ridgebackcontroller.reset()

    def is_done(self) -> bool:
        return self.event > 2

    # ------------------------------------------------------------------
    # Compatibility shims used by H1GR00TRunner
    # ------------------------------------------------------------------

    def forward(self, ik_method: str = "damped-least-squares"):
        """Advance the playbook one step. `ik_method` is unused (kept for API compat)."""
        self.play(testrun=True)

    def setup_mobile_base(self, stage):
        """No-op: the real RidgebackFranka is spawned separately in H1GR00TRunner.__init__."""
        print("[WarehousePlaybook] Using real RidgebackFranka articulation robot.")
        return None

    def command_callback(self, data):
        """Allow command center to override the Ridgeback target X position."""
        print("Message from command center..", data)
        if self.event == 0:
            self.ridge_targetlocation = [float(data), -1, 0]
...
class H1GR00TRunner(object):
    def __init__(
        ...
        # ── Spawn target cube (same as warehouse_play.py) ────────────────────
        try:
        ...
        self._cube = self._world.scene.add(
            DynamicCuboid(
                name="cube",
                position=np.array([cube_offset, -1, 0.029]),
        ...
        # ── Spawn the real RidgebackFranka articulation robot ────────────────
        self._ridgeback_robot = RidgebackFranka(
            prim_path='/World/Franka',
            name='my_franka',
            position=np.array([0, -1, 0])
        )
        self._world.scene.add(self._ridgeback_robot)

        # ── Reset world so all prims are on stage before configuring controller
        self._world.reset()

        # ── Configure pick/place controller and wire up the playbook ─────────
        pick_place_controller = RidgeBackFrankaPickPlace(
            cube=self._cube,
            robot=self._ridgeback_robot,
        )
        self._ridgeback_franka = WarehousePlaybook(pick_place_controller)
        print("[H1] Real RidgebackFranka articulation robot added to scene.")
        print("[H1] WarehousePlaybook (warehouse_play.py logic) wired up.")
    ...
    def _setup_static_franka_opposite(self, stage, cube_offset, assets_root_path):
        ...
        # The real RidgebackFranka is at /World/Franka
        active_franka_path = "/World/Franka"
    ...
    def on_physics_step(self, step_size) -> None:
        ...
        if self._franka_triggered_by_mqtt and not self._pick_place_active and not self._pick_place_done:
            ...
            self._ridgeback_franka.reset()  # reset WarehousePlaybook event counter
            print(f"[H1] Command center triggered Franka! Starting WarehousePlaybook pick-and-place...")
    ...
    def run(self) -> None:
        ...
        while simulation_app.is_running():
            ...
            if self.needs_reset:
                self._policy.reset()
                self._ridgeback_franka.reset()  # resets WarehousePlaybook + RidgeBackFrankaPickPlace
                self._object_detected = False
                self._pick_place_active = False
                self._pick_place_done = False
            ...
            if self._pick_place_active:
                self._ridgeback_franka.play(testrun=True)
...
def main():
    ...
    # NOTE: FrankaPickPlace is no longer used here.
    # The RidgebackFranka robot + DynamicCuboid + RidgeBackFrankaPickPlace
    # are now spawned inside H1GR00TRunner.__init__ using the warehouse_play.py approach.
